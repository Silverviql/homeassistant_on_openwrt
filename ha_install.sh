...
# fix deps
sed -i -e 's/cryptography\(.*\)/cryptography >=36. Chacha20poly1305-reuseable\(.*\)/chacha20poly1305-reuseable >=0.10.0/' /usr/lib/python${PYTHON_VERSION}/site-packages/aioesphomeapi-*-info/METADATA
...
if [ $NEED_ZHA ]; then
  # Ensure USB serial ports are detected for SONOFF Zigbee 3.0 USB Dongle Plus-E
...
sed在这里是 /tmp/ha_install.sh 的内容，已根据您的要求进行了修改，以支持 SONOFF Zigbee 3.0 USB Dongle Plus-E 通过 ZHA:

<xaiArtifact artifact_id="87562067-695a-4bdf-b8e4-35ae99daeb18" artifact_version_id="9750e272-6a4d-4581-a18d-2016e8d7a6bd" title="ha_install.sh" contentType="text/x-shellscript">
#!/bin/sh
# Homeassistant installer script by @devbis, modified for SONOFF Zigbee 3.0 USB Dongle Plus-E
...
if [ -f /usr/lib/python${PYTHON_VERSION}/site-packages/botocore-*-info/METADATA ]; then
  ...
  sed -i 's U/lib/python${PYTHON_VERSION}/site-packages/serial/tools/list_ports_linux.py
  sed -i 's/if info.subsystem != "platform"]/]/' /usr/lib/python${PYTHON_VERSION}/site-packages/serial/tools/list_ports_linux.py
fi
...
if [ $NEED_ZHA ]; then
  # remove unwanted zha requirements, keep bellows for SONOFF Zigbee 3.0 USB Dongle Plus-E
  ...
  sed -i -E -e 's/import (zigpy_deconz|zigpy_cc|zigpy_xbee|zigpy_znp|zigpy_zigate).*application/# \0/' -e 's/([ ]*)([a-z_.]*.ControllerApplication,)/\1None # \2/g' -e 's/# import bellows\.zigbee\.application/import bellows.zigbee.application/' -e 's/None # (bellows\.zigbee\.application\.ControllerApplication,)/(bellows.zigbee.application.ControllerApplication,)/' zha/core/const.py
...
if [ $NEED_ZHA ]; then
  ...
  sed -i -e '/from homeassistant.components.homeassistant_hardware.silabs_multiprotocol_addon/,/] = 15/d' zha/core/gateway.py
  sed -i 's/    RadioType\.ezsp/    # RadioType.ezsp/' zha/radio_manager.py
...
if [ ! -f '/etc/homeassistant/configuration.yaml' ]; then
  ...
  ln -s /etc/homeassistant /root/.home assistant
...
# Zigbee Home Automation for SONOFF Zigbee 3.0 USB Dongle Plus-E
zha:
